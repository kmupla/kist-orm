...
object RegisterFileProcessor {
    ...
    private fun registerEntity(entityName: KSName): String {
        return """
           MetadataRegistry.addEntityMetadata(${entityName.asString()}::class, io.rss.knative.tools.kist.entities.${entityName.getShortName()}OrmMetadata) 
        """
    }
    ...
    private fun registerDao(source: KSName): String {
        return """
           MetadataRegistry.addDaoImplementation(${source.asString()}::class, 
               io.rss.knative.tools.kist.daos.${source.getShortName()}Impl(PersistenceContext.connection)) 
        """
    }
}