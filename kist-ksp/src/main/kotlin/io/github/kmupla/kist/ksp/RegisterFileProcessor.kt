...
object RegisterFileProcessor {
    ...
    private fun registerEntity(entityName: KSName): String {
        return """
           MetadataRegistry.addEntityMetadata(${entityName.asString()}::class, io.github.kmupla.kist.entities.${entityName.getShortName()}OrmMetadata) 
        """
    }
    ...
    private fun registerDao(source: KSName): String {
        return """
           MetadataRegistry.addDaoImplementation(${source.asString()}::class, 
               io.github.kmupla.kist.daos.${source.getShortName()}Impl(PersistenceContext.connection)) 
        """
    }
}